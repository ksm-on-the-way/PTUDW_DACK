...
if (isset($_POST['action']) && $_POST['action'] === 'getShowsByDateAndLocation') {
    ...
    $query = '';
    if ($input_value !== '') {
        $query = "SELECT theaters.theater_name, theaters.theater_address, room_types.room_type_name, room_types.room_price, shows.start_time, shows.show_id
        FROM shows
        ...
        WHERE shows.date = '$date' AND theaters.city_id = $city_id  AND shows.movie_id = $movie_id  AND theaters.theater_name LIKE '%$input_value%'
        ...
        LEFT JOIN theaters ON rooms.theater_id = theaters.theater_id
        LEFT JOIN room_types ON rooms.room_type_id = room_types.room_type_id 
        WHERE shows.date = '$date' AND theaters.city_id = $city_id  AND shows.movie_id = $movie_id
        ORDER BY theaters.theater_name, room_types.room_type_name;
        ";
    }
    ...
    if ($result->num_rows > 0) {
        ...
        while ($row = $result->fetch_assoc()) {
            ...
            // Xuất start_time
            $html .= '<div class="time-item" cinema-type-name="' . $row["room_type_name"] . '" cinema-name="' . $row["theater_name"] . '" show-id="' . $row["show_id"] . '">' . substr($row["start_time"], 0, 5) . '</div>';
