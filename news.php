            ...
            if ($news_category_id !== null && $news_category_id !== '') {
                ...
                if ($result_news->num_rows > 0) {
                    ...
                    while ($row_news = $result_news->fetch_assoc()) {
                        echo "<article onclick = 'redirectToNewsDetail(" . $row_news['news_id'] . ")'>";
            ...
            <div class="collection_items">
                ...
                if ($result_collection->num_rows > 0) {
                    ...
                    while ($row_collection = $result_collection->fetch_assoc()) {
                        echo "<article onclick = 'redirectToNewsDetail(" . $row_collection['news_id'] . ")'>";
                    ...
                    if (searchQuery !== '') {
                        ...
                        xhr.onreadystatechange = function () {
                            if (xhr.readyState === XMLHttpRequest.DONE) {
                                if (xhr.status === 200) {
                                    // Trong hàm searchButton.addEventListener('click', function() { ... });
                                    if (xhr.responseText.trim() === 'Không tìm thấy tin tức.') {
                                        loadMoreNewsButton.style.display = 'none';
                                        loadMoreCollectionButton.style.display = 'none';
                                    } else {
                                        loadMoreNewsButton.style.display = 'none';
                                        loadMoreCollectionButton.style.display = 'none';
                                    }
            ...
            document.addEventListener("DOMContentLoaded", function () {
                ...
                loadMoreCollectionButton.addEventListener('click', function () {
                    ...
                    xhr.open('GET', './get_more_collection_items.php?offset=' + collectionOffset +
                        '&limit=' + limit + '&news_category_id=' + newsCategoryId, true);
                    xhr.send();
                });
            });
        </script>
